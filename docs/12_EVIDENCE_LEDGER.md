...
| **Serving stress test + hardening** | empirical edge-case probe; 2 load-path fixes (degraded service serves popularity not empty; real X/Y dim assert at startup); Cloud Run memory ~250–350 MB ALS-only | `[BUILT — empirical]` | `g_serving_stress_test.json`, `docs/PULSEDISCOVERY_FAILURES_AND_HARDENING.md` | "stress-tested the serving path, fixed two load-path gaps, sized for Cloud Run — production-sane within documented limits" |
| **☁ Cloud Run deployment (LIVE)** | served-c2 ALS + exact FlatIP deployed; live URL `pulsediscover-serving-98058433335.us-central1.run.app`; /health ready, /recommend source:als; 512MiB/1vCPU/min-instances 1; lean image (c2_als.pkl + 4MB assets) | `[DEPLOYED — live HTTPS]` | `deploy/` (cloudrun_deploy.sh, Dockerfile.cloudrun, DEPLOY.md), `scripts/build_serving_assets.py` | "deployed the FastAPI+FAISS serving path to GCP Cloud Run, serving live HTTPS — deployed ≠ online lift / real users / improved engagement" |
| **Numbers audit + precision fixes** | every V2 headline cross-checked vs source JSONs (all matched); fixed "42k items"→40,541 served catalog (k-core 41,961); popularity Gini stated as 0.9995 not 1.0; V1 numbers confirmed-in-artifacts but not re-derived this session | `[AUDIT]` | this ledger + `outputs/evidence/*.json` | "every claim verified against its evidence artifact and properly bounded; two precision wordings tightened" |
| **Portfolio presentation + GitHub publish** | dark portfolio README (badges + 2 SVGs) + 19-page dark defense PDF; requirements/.gitignore/.gcloudignore; **published to github.com/SidharthKriplani/pulsediscover (main)** | `[PUBLISHED]` | `README.md`, `docs/assets/*.svg`, `defense/PulseDiscover_Interview_Defense.pdf`, `scripts/build_defense_pdf.py` | "portfolio-grade README + comprehensive interview defense dossier, published" |
