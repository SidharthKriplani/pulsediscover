...
- **DEPLOYMENT NEW safe (LIVE):** *"I deployed the served-c2 ALS + exact-FlatIP serving path to GCP Cloud Run — versioned loader, fallback tree, health/metadata/metrics — and it serves live HTTPS (`/health`→ready, `/recommend`→source:als)."* **Forbidden:** "in production serving real users" · "improved engagement" · online lift · any business-impact claim. **Deployed & callable ≠ effective.**
- **AUDIT precision corrections (locked):** the served ALS catalog is **40,541 items** (k-core dataset total 41,961 ≈ "42k") — when asked "how many items does the model rank over?", answer **40,541**, not 42k. Popularity exposure Gini is **0.9995** (rounds to 1.0 but state 0.9995 for precision). Every V2 headline number was cross-checked vs its `outputs/evidence/*.json` source and matched. V1 numbers (0.0846, 0.065, 0.241, +44%) are confirmed-in-artifacts but **not re-derived from raw this session** (SASRec is `plot_reconstructed`, un-reproducible without GPU).
- **⚠ INTERVIEW VERBAL TRAPS (the materials are safe; do NOT say a stronger version aloud):** (1) "learned ranker beats ALS" → say *"beat ALS-only on the held-out test split, 0.036 vs 0.022, on 81 positives → directional, not production."* (2) "in production / serving users" → say *"deployed on Cloud Run, live HTTPS; deployed ≠ improved engagement."* (3) quoting "0.085" for the served model → say *"0.0846 is V1 tuning; served-c2 under V2 full-catalog single-gold is ~0.0375 — different protocols, registry-separated."* (4) "I trained a SASRec that got 0.065" → say *"converged and lost; direct artifact lost with the Colab session, tagged plot_reconstructed; the decision stands."* Source: `docs/PULSEDISCOVERY_INTERVIEW_KIT.md` + defense PDF Truth Boundary.
